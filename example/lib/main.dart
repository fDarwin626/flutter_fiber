    ...
    return MaterialApp(
      ...
      home: Scaffold(
        ...
        body: Fiber3DCanvas(
          ...
          orbitEnabled: true,
          lights: [
            const Fiber3DAmbientLight(color: 0x404040, intensity: 0.6),
            Fiber3DPointLight(
              color: 0xffffff,
              intensity: 8.0,
...
class SpinningSphere extends StatelessWidget {
  const SpinningSphere({super.key});

  @override
  Widget build(BuildContext context) {
    return Fiber3DMesh(
      geometry: Fiber3DSphere(radius: 1, widthSegments: 32, heightSegments: 16),
      ...
      onFrame: (elapsed, delta, transform) {
        transform.rotation.y += delta.inMicroseconds / 1e6;
      },
    );
  }
}