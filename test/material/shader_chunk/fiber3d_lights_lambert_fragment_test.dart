...
import 'package:flutter_fiber/src/material/shader_chunk/fiber3d_lights_lambert_fragment.dart';

void main() {
  const chunk = fiber3dLightsLambertFragment;

  group('fiber3dLightsLambertFragment', () {
    test('fills the LambertMaterial struct from diffuseColor', () {
      expect(chunk, contains('LambertMaterial material;'));
      expect(chunk, contains('material.diffuseColor = diffuseColor.rgb;'));
    });

    test('specularStrength defaults to 1.0 — flutter_fiber deviation, '
        'no specularmap_fragment ported yet to set a real value', () {
