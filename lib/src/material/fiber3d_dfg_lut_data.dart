/// Precomputed DFG lookup table for physically based specular lighting
/// used for both image-based lighting and the direct-light multi-scattering
/// energy compensation in `lights_physical_pars_fragment`.
///
/// Ported from three.js's `DFGLUT.js` (src/nodes/pmrem/DFGLUT.js). The
/// source stores 16-bit half-float hex pairs (scale, bias) for a 16x16
/// RG16F texture; [dfgLutData] decodes each to a double via
/// [halfToDouble], since Dart has no half-float type. The decoded values
/// are what gets uploaded as the texture's RG channels (as 32-bit floats,
/// since flutter_fiber has no half-float upload path yet).
library;

/// Raw half-float bit patterns, row-major, 16 columns x 16 rows, 2 values
/// (scale, bias) per texel copied unchanged from three.js's DATA array.
const List<int> _rawHalfFloats = [
...
/// Decodes an IEEE 754 half-precision (16-bit) float bit pattern to a
/// double. Handles the normal, subnormal, zero, infinity and NaN cases.
double halfToDouble(int bits) {
  ...
  if (exponent == 0) {
    // Subnormal or zero.
    value = fraction * (1 / 1024) * (1 / 16384); // fraction / 2^24
  } else if (exponent == 0x1f) {
    value = fraction == 0 ? double.infinity : double.nan;
  } else {
    final mantissa = 1 + fraction / 1024;
    value = mantissa * _pow2(exponent - 15);
  }
...
double _pow2(int exp) {
  double result = 1;
  final negative = exp < 0;
  final magnitude = exp.abs();
...
/// The DFG LUT decoded to doubles, one entry per (scale, bias) pair
/// 16 * 16 * 2 = 512 values, row-major, matching [_rawHalfFloats].
final List<double> dfgLutData = _rawHalfFloats.map(halfToDouble).toList();

/// LUT resolution (16x16), for both dimensions.
const int dfgLutSize = 16;