/// The physical (PBR) material model: the `PhysicalMaterial` struct, GGX
/// distribution and Smith visibility, `BRDF_GGX`, rect-area LTC helpers,
/// sheen/clearcoat/anisotropy/iridescence/EON variants (all behind their
/// #ifdef guards), environment BRDF via the DFG lookup texture, multi-
/// scattering, and the `RE_Direct` / `RE_IndirectDiffuse` /
/// `RE_IndirectSpecular` entry points.
///
/// Ported from three.js's `lights_physical_pars_fragment.glsl.js`
/// (src/renderers/shaders/ShaderChunk/), copied unchanged except that two
/// comments were made plain ASCII (desktop GLSL 150 compilers can reject
/// other characters). Depends on `common`, `lights_pars_begin`
/// (RectAreaLight), the `dfgLUT` texture, and the `texture2D` macro.
const String fiber3dLightsPhysicalParsFragment = r'''
...
// Fdez-Aguera's "Multiple-Scattering Microfacet Model for Real-Time Image Based Lighting"
